...
Jewelry and scrap workflows usually start from the same three inputs: how heavy the piece is, what karat stamp it carries, and what fine gold is trading at today. This library keeps that math in one place so a Java app or script can reuse the same rounding rules.
...
```xml
<dependency>
  <groupId>io.github.ilovejackylee</groupId>
  <artifactId>gold-karat-value</artifactId>
...
```java
import io.github.ilovejackylee.goldkaratvalue.Calculator;
import io.github.ilovejackylee.goldkaratvalue.WeightUnit;

var melt = Calculator.meltValue(10, WeightUnit.GRAM, 14, 65);
System.out.printf("14K melt: $%.2f ($%.2f/g)%n", melt.total, melt.perGram);

var scrap = Calculator.scrapValue(10, WeightUnit.GRAM, 14, 65, 0.85);
System.out.printf("85%% scrap offer: $%.2f%n", scrap.scrap);
```
